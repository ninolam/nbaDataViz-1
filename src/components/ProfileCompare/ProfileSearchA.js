...
class ProfileSearchB extends Component {
  ...
  filterList(event) {
    if (event.target.value !== "") {
      ...
      document.querySelector('.PlayerA ul').style.visibility = "visible"
    }
    else {
      document.querySelector('.PlayerA ul').style.visibility = "hidden"
    }
  ...
  onChangeupdateItemA(newItem) {
    this.setState({ updateItemA: newItem })
  }
  ...
  render() {
    if (this.state.ProfileResult === false) {
      ...
      return (
        <ProfileResultA renderSearch={this.renderSearch()} updateItemA={this.state.updateItemA} onChangeupdateItemA={this.onChangeupdateItemA.bind(this)} />
      );
    }
  }
        ...
        <ul className="list-group">
          {
            this.state.items.map((item, i) => {
              ...
              return <li className="list-group-item" style={{ backgroundImage: "url(" + "https://tsnimages.tsn.ca/ImageProvider/PlayerHeadshot?seoId=" + item.replace(' ', '-').toLowerCase() + ")" }}
                onMouseOver={(e) => { var list = document.querySelectorAll('.PlayerA ul li'); list[i].textContent = item; list[i].style.opacity = 0.5 }}
                onMouseOut={() => { var list = document.querySelectorAll('.PlayerA ul li'); list[i].textContent = ""; list[i].style.opacity = 1 }}
                onClick={(event) => { const Players = this.state.initialItems.filter(player => player.name === event.currentTarget.dataset.category); this.setState({ updateItemA: Players[0], ProfileResult: true }); var inputValue = document.querySelector('.PlayerA .Search-all-container input'); inputValue.value = Players[0].name }}
